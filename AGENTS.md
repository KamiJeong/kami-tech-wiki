...
이 파일은 LLM(Codex)이 이 위키를 유지할 때 따라야 할 모든 규칙을 정의합니다.
...
7. **Git 커밋**: 커밋 메시지에 `Co-Authored-By: Codex` 등 Codex 작성 표기를 포함하지 않는다.
...
| 페이지 유형 | 사용할 템플릿 |
|------------|--------------|
| AI 도구 개요 (ChatGPT, Codex 등) | `.wiki/templates/tool-page.md` |
