...
- **FR-001**: `wiki/ko/openui/`, `wiki/en/openui/`, `wiki/ja/openui/` 디렉토리 아래에 각각 OpenUI 공식 문서의 모든 주요 섹션에 대응하는 위키 페이지를 생성해야 한다. 한국어(ko)가 기준 언어이며, 영어(en)와 일본어(ja)는 한국어 콘텐츠를 기반으로 현지화한다.
...
- **SC-001**: OpenUI 공식 문서의 12개 이상 페이지가 한국어(ko), 영어(en), 일본어(ja) 세 언어로 각각 내부 위키에 생성된다 (총 36페이지 이상).
...
- Q: 영어(en)와 일본어(ja) 위키를 어떻게 처리할까요? → A: 현재 `004-openui-docs` feature에 영어(en) + 일본어(ja) 모두 추가. 한국어 콘텐츠 기반으로 번역·현지화하여 동일 PR에서 관리.

## Assumptions

- 내부 위키의 기준 언어는 한국어(ko)이며, 영어(en)와 일본어(ja)도 동일 feature에서 함께 생성한다. 한국어 콘텐츠를 먼저 작성하고 이를 기반으로 현지화한다.
